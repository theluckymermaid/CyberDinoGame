...
//	InvokeRepeating("RandomSpawn", .5, .1);
...
function RandomSpawn() {
	var p:int = Random.Range(0,particles.Length);
	if(p!=11&&p!=10&&p!=33&&p!=32){
	var pp:GameObject = Instantiate(particles[p], Vector3(Random.Range(175.0, -175.0), Random.Range(175.0, -175.0), Random.Range(0, 550.0)), transform.rotation);
...
function StopEmit(pp:GameObject){

	yield(WaitForSeconds(pp.GetComponent.<ParticleSystem>().duration));
...
function OnGUI () {
	...
	if(showInfo)GUI.Label (Rect(buttonWidth+hSpace+hSpace, hSpace*3,500,500), currentGOInfo, bigStyle);
	if(!android){
	...
	if(particles.length > maxButtons){
		//Prev button
		if(GUI.Button(Rect(hSpace,(maxButtons+1)*18,buttonWidth*.5,18),"Prev"))if(page > 0)page--;else page=pages;
		//Next button
		if(GUI.Button(Rect(buttonWidth*.5+hSpace,(maxButtons+1)*18,buttonWidth*.5,18),"Next"))if(page < pages)page++;else page=0;
		//Page text
		GUI.Label (Rect(60,(maxButtons+2)*18,150,22), "Page" + (page+1) + " / " + (pages+1));
		
	}
	//Toggle button for info
	showInfo = GUI.Toggle (Rect(buttonWidth+hSpace+hSpace, hSpace,buttonWidth*.5,25), showInfo, "Info");
	...
	for(var i:int=0;i < pageButtonCount;i++){
		...
		if(GUI.Button(Rect(hSpace,i*18+18,buttonWidth,18),buttonText)){
	...
	for(var m:int=0;m < materials.Length;m++){
		...
		if(GUI.Button(Rect(hSpace,(maxButtons+m+4)*18,150,18),b)){
	...
	if(GUI.Button(Rect((Screen.width*.5)-150,Screen.height-120,150,100),"Prev")){
	...
	if(GUI.Button(Rect((Screen.width*.5),Screen.height-120,150,100),"Next")){
			NextModel();
