...
public class MappedInput : MonoBehaviour {
    ...
    static CollatedInputDevice _allDevices = null;
    public static CollatedInputDevice allDevices
    {
        get
        {
            return _allDevices;
	...
	void Awake()
	{
        ...
        // Don't create a KeyboardDevice for mobile devices!        
	...
	void OnGamepadRemoved(GamepadDevice gamepad)
	{
		for(int i = 0; i < inputDevices.Count; i++)
		{
			if( inputDevices[i] is GamepadInputDevice && (inputDevices[i] as GamepadInputDevice).gamepad == gamepad)
			{
                ...
                if (_allDevices != null)
                    _allDevices.RemoveDevice(device);
    ...
    void AddCollatedDevice() {
        GameObject obj = new GameObject("Collated", typeof(CollatedInputDevice));
        obj.transform.parent = transform;
        CollatedInputDevice collatedDevice = obj.GetComponent<CollatedInputDevice>();
        foreach (InputDevice device in inputDevices) {
            collatedDevice.AddDevice(device);
        }
        inputDevices.Add(collatedDevice);

        _allDevices = collatedDevice;
    }
}