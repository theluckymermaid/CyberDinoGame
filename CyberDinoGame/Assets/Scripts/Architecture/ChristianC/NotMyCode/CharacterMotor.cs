...
public class CharacterMotor : MonoBehaviour
{
    ...
    private void UpdateFunction()
    {
        ...
        {
            //This 
            float projectedNewVelocity = Mathf.Max(Vector3.Dot(newHVelocity, oldHVelocity) / oldHVelocity.magnitude,0);
            //Debug.Log(projectedNewVelocity + " " + newHVelocity);
            //movement.velocity = newHVelocity * Mathf.Clamp01(projectedNewVelocity) + movement.velocity.y * Vector3.up;

            //If we're suddenly going in an opposite direction, don't apply previous velocity to our new direction.
            if (projectedNewVelocity != 0)
                movement.velocity = (newHVelocity.normalized * projectedNewVelocity) + (movement.velocity.y * Vector3.up);
        }
        ...
        else if(!grounded && IsGroundedTest())
        {
            ...
            StartCoroutine(SubtractNewPlatformVelocity()); // Was not working before
    ...
    {
        Debug.Log("SubtactNewPlatformVelocity() ran!");
