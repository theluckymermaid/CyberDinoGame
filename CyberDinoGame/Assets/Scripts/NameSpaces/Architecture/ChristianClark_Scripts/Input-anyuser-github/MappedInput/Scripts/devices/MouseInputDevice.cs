	...
	protected override float GetAxisValueRaw (MappedAxis axis)
	{
		...
		if (mapping != null)
		{
            if (mapping.mouseAxisId == 0) {
                rawVal = Input.GetAxis("Mouse X");
            } else if (mapping.mouseAxisId == 1) {
                rawVal = Input.GetAxis("Mouse Y");
            } else if (mapping.mouseAxisId == -1) {
                ...
                rawVal = Mathf.Clamp(rawVal, -1f, 1f);
            }
                else throw new UnityException("Mouse axis " + mapping.mouseAxisId + " not found");
		}
